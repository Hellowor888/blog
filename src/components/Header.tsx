...
export default function Header({ darkToggle, onSearch }: HeaderProps) {
  ...
  const linkClass = (path: string) =>
    `transition-all duration-300 px-2.5 py-1.5 rounded-lg text-sm font-medium ${
      pathname === path
    ...
    <header className="sticky top-0 z-20 backdrop-blur-xl bg-white/70 dark:bg-gray-950/70 border-b border-gray-200/50 dark:border-gray-800/50">
      <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-0.5">
          ...
          <button onClick={onSearch} className="px-2.5 py-1.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-all duration-300" title="搜索 (Ctrl+K)">
